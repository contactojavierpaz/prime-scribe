...
export default function PrimeScribe() {
  ...
  // Chat
  const [imagenAdjunta, setImagenAdjunta] = useState(null);   // { data, type, preview }
  ...
  async function enviarMensaje(texto) {
    ...
    if (dictando) { reconocimiento.current?.stop(); setDictando(false); }

    const mensajeUsuario = imagenAdjunta
      ? { role: 'user', content: contenido || 'Describe lo que observas en esta imagen.',
          image: imagenAdjunta.data, imageType: imagenAdjunta.type, preview: imagenAdjunta.preview }
      : { role: 'user', content: contenido };

    const nuevos = [...mensajes, mensajeUsuario];
    setMensajes(nuevos); setEntrada(''); setImagenAdjunta(null); setPensando(true);
        ...
        {tab === 'chat' && (
          <div className="card">
            <div className="card-head">
              ...
              {mensajes.length > 0 && (
                <button className="btn btn-ghost btn-sm" onClick={guardarChatComoNota} disabled={guardandoChat}>
                  {guardandoChat ? <><span className="spinner" /> Guardando...</> : '💾 Guardar en el expediente'}
                </button>
              )}
            </div>

            {alertas.filter(a => a.nivel === 'roja').length > 0 && (
              <div className="alert alert-red" style={{ marginBottom: '1rem' }}>
                <span>⛔</span>
                <span>
                  Este paciente tiene {alertas.filter(a => a.nivel === 'roja').length} alerta
                  {alertas.filter(a => a.nivel === 'roja').length > 1 ? 's' : ''} roja
                  {alertas.filter(a => a.nivel === 'roja').length > 1 ? 's' : ''}. El asistente las considera en cada respuesta clínica.
                </span>
              </div>
            )}
            ...
            <div className="chat-log" ref={chatRef}>
              {mensajes.length === 0 && (
                <div className="empty">Escribe una pregunta o elige una sugerencia.</div>
              )}
              {mensajes.map((m, i) => (
                <div key={i} className={`msg ${m.role}`}>
                  <div className="msg-bubble">
                    {m.preview && (
                      <img src={m.preview} alt="Imagen adjunta"
                        style={{ maxWidth: '100%', borderRadius: 6, marginBottom: 8, display: 'block' }} />
                    )}
                    {m.content}
                  </div>
                </div>
            ...
            {imagenAdjunta && (
              <div className="img-preview">
                <img src={imagenAdjunta.preview} alt="Vista previa" />
                <div className="grow">
                  <div style={{ fontSize: 12, color: 'var(--dark)' }}>{imagenAdjunta.nombre}</div>
                  <div style={{ fontSize: 11, color: 'var(--muted)' }}>Se enviará con tu mensaje</div>
                </div>
                <button className="btn btn-ghost btn-sm" onClick={quitarImagen}>Quitar</button>
              </div>
            )}

            <div className="chat-input-row">
              <input type="file" accept="image/*" ref={fileInput}
                onChange={elegirImagen} style={{ display: 'none' }} />

              <button className="icon-btn" onClick={() => fileInput.current?.click()}
                title="Adjuntar imagen" disabled={pensando}>
              ...
              <button className={`icon-btn ${dictando ? 'active' : ''}`} onClick={alternarDictado}
                title={dictando ? 'Detener dictado' : 'Dictar por voz'} disabled={pensando}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M12 2a3 3 0 00-3 3v6a3 3 0 006 0V5a3 3 0 00-3-3z" />
                  <path d="M19 10v1a7 7 0 01-14 0v-1" /><line x1="12" y1="18" x2="12" y2="22" />
                </svg>
            ...
            {dictando && (
              <div style={{ fontSize: 11, color: 'var(--red)', marginTop: 6, textAlign: 'center' }}>
                🎙️ Dictando — toca el micrófono para detener
              </div>
            )}
          </div>
        )}
